...
<head>
    ...
    <title>Senjata Adat</title>
</head>
<body>
    <?php include('navbar.php'); ?>

    <div class="image-header" style="background-image: url(img/senjataadat.jpg)">
        <div class="tirai"></div>
    ...
    <!-- Mulai Konten -->
    <h1 class="text-center">Senjata Adat di Indonesia</h1>
    <br />
    <style>
        .card {
            height: 360px;
    ...
    <div class="container">
        <div class="row justify-content-center" id="SenjataContainer">
            <?php
            // Mengambil data dari tabel senjataadat
            $sql = "SELECT Senjata, SenjataIMG, DesSenjataAdat FROM senjataadat";
            $result = $conn->query($sql);

            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    $namaSenjata = $row["Senjata"];
                    $deskripsiSenjata = $row["DesSenjataAdat"];
                    $imgData = base64_encode($row["SenjataIMG"]);
                    echo "<div class=\"col-lg-4 col-md-4 col-sm-6 mb-4\">";
                    echo "  <div class=\"card h-custom\" data-toggle=\"modal\" data-target=\"#imageModal\" data-name=\"$namaSenjata\" data-img=\"$imgData\" data-description=\"" . htmlspecialchars($deskripsiSenjata) . "\">";
                    echo "    <img src=\"data:image/jpeg;base64,$imgData\" class=\"card-img-top img-kuliner\" style=\"object-fit: cover;\">";
                    echo "    <div class=\"card-body\">";
                    echo "      <h5 class=\"card-title\">${namaSenjata}</h5>";
                    echo "    </div>";
                    echo "  </div>";
                    echo "</div>";
                }
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="imageModalLabel">Detail Senjata Adat</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body text-center">
                    <img id="modalImage" src="" class="img-fluid modal-image" alt="Gambar Senjata Adat">
