...
<body>
    ...
    <div class="image-header" style="background-image: url(img/rumahadat.jpg)">
        <div class="tirai"></div>
    ...
    <div class="container">
        <div class="row justify-content-center" id="RumahContainer">
            <?php
            // Mengambil data dari tabel rumahadat
            $sql = "SELECT Rumah, RumahIMG, DesRumahAdat FROM rumahadat";
            $result = $conn->query($sql);

            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    $namaRumah = $row["Rumah"];
                    $deskripsiRumah = $row["DesRumahAdat"];
                    $imgData = base64_encode($row["RumahIMG"]);
                    echo "<div class=\"col-lg-4 col-md-4 col-sm-6 mb-4\">";
                    echo "  <div class=\"card h-custom\" data-toggle=\"modal\" data-target=\"#imageModal\" data-name=\"$namaRumah\" data-img=\"$imgData\" data-description=\"" . htmlspecialchars($deskripsiRumah) . "\">";
                    echo "    <img src=\"data:image/jpeg;base64,$imgData\" class=\"card-img-top img-kuliner\" style=\"object-fit: cover;\">";
                    echo "    <div class=\"card-body\">";
                    echo "      <h5 class=\"card-title\">${namaRumah}</h5>";
                    echo "    </div>";
                    echo "  </div>";
                    echo "</div>";
                }
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="imageModalLabel">Detail Rumah Adat</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body text-center">
                    <img id="modalImage" src="" class="img-fluid modal-image" alt="Gambar Rumah Adat">
